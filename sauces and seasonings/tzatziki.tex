...
\noindent
    ...
    Full-fat goat milk yogurt \dotfill 3 cups \\
    Garlic cloves \dotfill 5 \\
    Red wine vinegar \dotfill 1\nicefrac{1}{2}-2 Tbsp. \\
    \columnbreak
    Extra virgin olive oil \dotfill 3-4 Tbsp. \\
    Fresh dill, chopped \dotfill 2 Tbsp. \\
    Fresh mint, chopped \dotfill 2 Tbsp. \\
...
\noindent
Grate \textbf{cucumber} on large holes of box grater; set aside in \textit{Medium Bowl~\#1} ---
Very finely mince \textbf{garlic}; set aside in \textit{Small Bowl~\#1} ---
Chop \textbf{dill} and \textbf{mint}; combine in \textit{Small Bowl~\#2}
...
\begin{enumerate}
    ...
    \item After draining, gather \textbf{cucumber} (\textit{Medium Bowl~\#1}) in a clean kitchen towel (or multiple layers of cheesecloth) and squeeze aggressively to extract maximum liquid. Wring and twist until you cannot extract any more. You should remove \nicefrac{3}{4}-1~cup liquid. The \textbf{cucumber} should feel almost dry when finished. Return to \textit{Medium Bowl~\#1}; set aside.
    
    \item Grate or very finely mince \textbf{garlic} (\textit{Small Bowl~\#1}). For more even distribution and less harsh bite, grate on a microplane. For stronger garlic punch with some texture, mince very fine with a knife. Optionally, crush \textbf{garlic} with a pinch of \textbf{salt} and \textbf{pepper} to create a paste.
    
    \item In \textit{Medium Bowl~\#2}, add 3~cups \textbf{goat milk yogurt} and prepared \textbf{garlic} (\textit{Small Bowl~\#1}). Begin adding \textbf{olive oil} gradually, 1~tablespoon at a time, while stirring vigorously. Alternate with small additions of \textbf{red wine vinegar} (1\nicefrac{1}{2}-2~Tbsp.). This slow incorporation prevents the oil from pooling on top and creates a smooth, emulsified texture. Add \nicefrac{1}{2}~tsp. \textbf{kosher salt} and mix thoroughly.
    
    \item Add thoroughly drained \textbf{cucumber} (\textit{Medium Bowl~\#1}), chopped \textbf{dill} and \textbf{mint} (2~Tbsp. each, \textit{Small Bowl~\#2}) to \textbf{yogurt} mixture (\textit{Medium Bowl~\#2}). Fold together until evenly combined. The mixture should be very thick.
