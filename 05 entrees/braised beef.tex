...
Rough chop \textbf{onions} and \textbf{celery} into large chunks; combine in \textit{Large Bowl~\#1} ---
...
\begin{enumerate}
    \item Season \textbf{beef chuck} generously on all sides with 1\nicefrac{1}{2}~tsp. \textbf{kosher salt} and \nicefrac{1}{2}~tsp. \textbf{black pepper}.
    
    \item In a Dutch oven, heat 2~Tbsp. \textbf{vegetable oil} over medium-high heat until shimmering. Working carefully to avoid splatter, sear the \textbf{beef} on all sides until deeply browned, approximately \textit{3-4~minutes} per side. The goal is a rich mahogany crust. Remove \textbf{beef} and set aside in \textit{Large Bowl~\#2}.
    
    \item In the same pot, reduce heat to medium. Add rough-chopped \textbf{onions} and \textbf{celery} (\textit{Large Bowl~\#1}), stirring occasionally until they begin to soften and caramelize slightly, about \textit{5-7~minutes}. Add minced \textbf{garlic} (\textit{Small Bowl~\#1}) and cook for \textit{1~minute} until fragrant.
    
    \item Add 3~Tbsp. \textbf{tomato paste} and stir constantly for \textit{1-2~minutes}, allowing it to caramelize slightly and deepen in color. This develops savory complexity.
    
    \item Deglaze the pot with 2~cups \textbf{Burgundy wine}, scraping up all browned fond from the bottom with a wooden spoon. Simmer for \textit{2-3~minutes} to allow alcohol to burn off slightly.
    
    \item Return seared \textbf{beef} (\textit{Large Bowl~\#2}) to the pot. Add 2~cups \textbf{beef stock} and 2~Tbsp. \textbf{French dip seasoning mix}. The liquid should come approximately one-third up the sides of the meat. Add \textbf{bouquet garni}.
\end{enumerate}
