...
\begin{multicols}{2}
\noindent
    Chicken thighs, \dotfill 2-2½ lbs \\
    Yellow onion, large \dotfill 1 \\
    Garlic cloves \dotfill 4-6 \\
    Mexican spice blend \dotfill 4 Tbsp \\
    Salsa \dotfill 7 oz. can \\
    \columnbreak
...
\begin{enumerate}
    ...
    \item Add 4~Tbsp \textbf{Mexican~spice~blend} to the \textbf{onions}. Cook \textit{45-60~seconds}, stirring constantly, until a toasted fragrance develops. You will smell the distinct shift from raw spice to bloomed aromatic. Immediately add the entire 7~oz. can of \textbf{Herdez~salsa~ranchera} to stop the bloom. Scrape all the fond from the bottom of the pot—this concentrated layer is essential for depth.
    
    \item Add minced \textbf{garlic} to the tomato mixture and cook for \textit{30~seconds}. Add \textbf{chicken~stock}, \textbf{caldo~de~tomate}, and \textbf{bay~leaves}. Stir well and bring to a simmer. Taste the braising liquid—it should be intensely flavored and slightly too salty. If not sufficiently seasoned, add another ½~Tbsp \textbf{caldo~de~tomate}.
    ...
    \item Check at \textit{2½~hours}: the meat should pull from the bone with zero resistance. If still slightly firm, continue braising for another \textit{30~minutes}. Bone-in thighs are very forgiving and can go to \textit{3+~hours} without harm.
    
    \item Remove \textbf{chicken} from the pot and let cool for \textit{10~minutes} to make handling easier. Pick all meat from the bones—it will slide off easily. Discard skin and bones. Pour the braising liquid through a fine mesh strainer into a saucepan, discarding solids.
    
    \item Tilt the saucepan and use a spoon to skim excess fat from the surface. Reserve 2~Tbsp of this rendered \textbf{chicken~fat} in a small bowl; discard the rest. Place the saucepan over medium-high heat and reduce the liquid by half, which should take \textit{10-15~minutes}. You want approximately 1½-2~cups final volume. The sauce should look glossy and coat the back of a spoon. Taste periodically as flavor intensifies dramatically during reduction.
    
    \item Shred the \textbf{chicken} into bite-size pieces—not too fine, maintain some texture. Add shredded \textbf{chicken} to the reduced sauce along with 2~Tbsp \textbf{lard}. Fold gently to coat every piece.
    
    \item Add juice of 2-3~\textbf{limes} (start with 2, taste, then add more if needed) and ½~cup chopped \textbf{cilantro}. Fold to combine. Taste and adjust \textbf{salt} and \textbf{lime~juice} as needed. The final result should taste deeply savory with subtle smoke, bright acidity, and rich mouthfeel. The sauce should cling to every shred without pooling.
