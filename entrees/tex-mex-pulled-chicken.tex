...
\fancyhead[C]{\textit{Tex-Mex Pulled Chicken}} % Center header with recipe title
...
\begin{enumerate}
    ...
    \item Add minced \textbf{garlic} (\textit{Small Bowl~\#2}) to the tomato mixture and cook for \textit{30~seconds}. Add 2\nicefrac{1}{2}~cups \textbf{chicken stock}, 2~Tbsp. \textbf{caldo de tomate}, and 2 \textbf{bay leaves}. Stir well and bring to a simmer. Taste the braising liquid—it should be intensely flavored and slightly too salty. If not sufficiently seasoned, add another \nicefrac{1}{2}~Tbsp. \textbf{caldo de tomate}.
    
    \item Return \textbf{chicken thighs} (\textit{Large Bowl~\#1}) to the pot skin-side up. Liquid should come halfway up the \textbf{chicken}; add more \textbf{stock} if needed. Bring to a bare simmer on the stovetop, then cover with a tight-fitting lid. Transfer to the preheated \textit{275°F} oven and braise for \textit{2\nicefrac{1}{2}-3~hours}.
    
    \item Check at \textit{2\nicefrac{1}{2}~hours}: the meat should pull from the bone with zero resistance. If still slightly firm, continue braising for another \textit{30~minutes}.
    
    \item Remove \textbf{chicken} from the pot and let cool for \textit{10~minutes} to make handling easier. Remove skin and bones, etc. Transfer to \textit{Large Bowl~\#2}.
    
    \item In \textit{Large Bowl~\#3}, shred the \textbf{chicken} (\textit{Large Bowl~\#2}) into bite-size pieces, maintaining some texture. Add \nicefrac{1}{2}~cup of the \textbf{braising liquid}, 2~Tbsp. \textbf{reserved chicken fat} (\textit{Small Bowl~\#5}), \textbf{lime juice} (\nicefrac{1}{4}~cup, \textit{Small Bowl~\#4}), and \nicefrac{1}{2}~cup \textbf{cilantro} (\textit{Small Bowl~\#3}). Fold gently to combine. Add more \textbf{braising liquid} as needed until moist but not soupy.
